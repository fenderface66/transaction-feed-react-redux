...
import { getTransactions, githubData } from '../sagas';
...
describe('getTransactions Saga', () => {
  ...
  beforeEach(() => {
    ...
    const callDescriptor = getTransactionsGenerator.next(username).value;
  ...
  it('should dispatch the transactionsLoaded action if it requests the data successfully', () => {
    ...
    const putDescriptor = getTransactionsGenerator.next(response).value;
    expect(putDescriptor).toEqual(put(transactionsLoaded(response, username)));
  });
